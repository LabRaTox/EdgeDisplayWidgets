import { registerWidget } from "../registry.js";
import { Sparkline } from "../lib/sparkline.js";
import { Smoothed } from "../lib/smooth.js";
import { t } from "../i18n.js";

class CpuWidget {
  static modules = ["system"];
  // Offered in the layout editor's variant picker; the backend reads this
  // declaration straight out of this file (see _widget_variants in main.py).
  static variants = ["compact"];

  mount(el, _, ctx) {
    this.el = el;
    // `compact` leaves the chart and the core bars out of the DOM instead of
    // hiding them with CSS. That is the point of the variant: a sparkline
    // that is not there costs nothing, while a hidden one still redraws on
    // every sample — and the core bars were the most expensive thing on the
    // whole dashboard.
    this.compact = ctx?.variant === "compact";
    el.innerHTML = `
      <div class="metric-head">
        <h3>CPU</h3>
        <div class="metric-big" data-bind="percent">–</div>
      </div>
      <div class="metric-sub" data-bind="sub">–</div>
      ${this.compact ? "" : `
      <div class="chart">
        <div class="chart-axis"></div>
        <div class="chart-canvases">
          <canvas class="spark"></canvas>
        </div>
      </div>
      <div class="cpu-cores" data-bind="cores_bars"></div>
      `}
    `;
    this.spark = this.compact
      ? null
      : new Sparkline(el.querySelector(".spark"), {
          max: 100,
          axisEl: el.querySelector(".chart-axis"),
          axisFormat: (v) => `${Math.round(v)}%`,
          scroll: true,
        });
    this._coreEls = null;
    const percentEl = el.querySelector('[data-bind="percent"]');
    this._percent = new Smoothed((v) => {
      percentEl.textContent = v.toFixed(0) + "%";
    }, { el });
  }

  update(data) {
    const cpu = data?.cpu;
    if (!cpu) return;
    this._percent.set(cpu.percent);
    const parts = [];
    if (cpu.model) parts.push(cpu.model);
    parts.push(`${cpu.count} ${t("widget.cpu.cores")}`);
    if (cpu.freq_mhz) parts.push(`${(cpu.freq_mhz / 1000).toFixed(2)} GHz`);
    this.el.querySelector('[data-bind="sub"]').textContent = parts.join(" • ");
    if (this.compact) return;
    this.spark.push(cpu.percent);
    this._renderCores(cpu.per_core || []);
  }

  _renderCores(values) {
    const root = this.el.querySelector('[data-bind="cores_bars"]');
    if (!this._coreEls || this._coreEls.length !== values.length) {
      root.innerHTML = "";
      this._coreEls = values.map(() => {
        const wrap = document.createElement("div");
        wrap.className = "core-bar";
        const fill = document.createElement("div");
        fill.className = "core-fill";
        wrap.appendChild(fill);
        root.appendChild(wrap);
        return fill;
      });
    }
    // One write per sample, and the `transition: height` on `.core-fill` does
    // the animating. Interpolating these in JavaScript was measurably the most
    // expensive thing on the whole dashboard: 32 bars x 60 fps is ~1900 style
    // writes a second, each one restarting the very CSS transition it was
    // duplicating. Removing it took the kiosk from 57% to 28% CPU.
    for (let i = 0; i < values.length; i++) {
      const v = Math.max(0, Math.min(100, values[i]));
      this._coreEls[i].style.height = v + "%";
    }
  }

  destroy() {
    this.spark?.destroy();
    this._percent.destroy();
  }
}

registerWidget("cpu", CpuWidget);
